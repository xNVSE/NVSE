...
namespace Compiler {
	...
	const UInt32 g_messageBoxParseCmds[] = {
	...
	bool doesFormMatchParamType(TESForm* form, const ParamType type) {
		...
		case kParamType_MapMarker:
			if (!form || NOT_ID(form, TESObjectREFR) || (((TESObjectREFR*)form)
		...
		case kParamType_Cell:
			if (!form || NOT_ID(form, TESObjectCELL) || !(((TESObjectCELL*)form)
				->cellFlags & 1)) {
				return false;
			}
			break;
