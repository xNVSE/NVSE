...
ScriptEventList* ResolveExternalVar(ScriptEventList* in_EventList, Script* in_Script, UInt8* &scriptData)
{
	...
	Script::RefVariable* refVar = in_Script->GetRefVariable(varIdx);
	if (refVar)
	{
...
TESGlobal* ResolveGlobalVar(ScriptEventList* in_EventList, Script* in_Script, UInt8* &scriptData)
{
	...
	Script::RefVariable* globalRef = in_Script->GetRefVariable(varIdx);
	if (globalRef)
...
//	scriptData += 2;
	...
	case 'r':
		{
			Script::RefVariable	* var = scriptObj->GetRefVariable(varIdx);
			if(var)
			{
	...
	case 'G':						//global
		{
			...
			Script::RefVariable* refVar = script->GetRefVariable(refIdx);
			if (!refVar)
				break;
